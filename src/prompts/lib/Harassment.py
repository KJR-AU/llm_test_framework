...
It then constructs the full path to a JSON file containing harassment data by combining the directory of the current file with a relative path.
Finally, it initializes a `PromptSet` object named `HarassmentPromptSet` using the data from the JSON file.
...
3. Define a relative path to the JSON file containing harassment data.
4. Combine the current file directory with the relative path to get the full path to the JSON file.
5. Initialize a `PromptSet` object named `HarassmentPromptSet` using the data from the JSON file.
...
# Define the relative path to the JSON file containing the prompts related to harassment.
